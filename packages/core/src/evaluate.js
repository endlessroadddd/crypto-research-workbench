...
const confidenceBandForScore = (score) => {
...
const determineModes = (candidate) => {
    const modes = [];
...
export const evaluateCandidate = (input, context) => {
    const freshEvidence = applyFreshness(input.evidence, context.now, defaultRouterConfig.freshnessDegradingFloor);
    ...
    const longBreakdown = calculateScoreBreakdown(baseCandidate, "bullish", resonance, lifecycleStage, regimeState);
    const shortBreakdown = calculateScoreBreakdown(baseCandidate, "bearish", resonance, lifecycleStage, regimeState);
    ...
    if (shortBreakdown.finalScore > longBreakdown.finalScore &&
    ...
    return {
        ...
        decisionReason: chosenReasons.concat(chosenBreakdown.vetoState !== "none" ? [`veto:${chosenBreakdown.vetoState}`] : [], conflictSeverity !== "none" ? [`conflict:${conflictSeverity}`] : [], chosenDecision === "watchlist" ? ["needs_confirmation"] : [], chosenDecision === "observe_cooloff_15m" ? ["cooloff_due_to_conflict"] : [])
    };
};
