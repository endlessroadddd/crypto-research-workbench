...
export const routeLana = (candidate: Candidate): RouterDecision => {
  ...
  const hasSocialOnly =
    candidate.activeEvidence.some(
      (item) =>
        ["social_sentiment", "rank_aggregator"].includes(item.sourceFamily) &&
        item.direction === "bullish"
    ) && !hasStructureConfirmation;
  const hasStrongDiscovery =
    breakdown.discoveryScore >= defaultRouterConfig.thresholds.watchlistScore ||
