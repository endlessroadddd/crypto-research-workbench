import type { Evidence, RegimeState, SourceCoverageItem } from "./types";
import { coreStructureSources } from "./config";

const hasCoreStructureDegradation = (sourceCoverage: SourceCoverageItem[]): boolean =>
  sourceCoverage.some(
    (item) =>
      coreStructureSources.includes(item.name) &&
      (item.status === "stale" ||
...
export const classifyRegime = (
  ...
  const bullishCrowding = score(
    evidence,
    (item) =>
      ["social_sentiment", "rank_aggregator"].includes(item.sourceFamily) &&
      item.direction === "bullish"
  );
  const bearishCrowding = score(
    evidence,
    (item) =>
      ["social_sentiment", "rank_aggregator"].includes(item.sourceFamily) &&
      item.direction === "bearish"
