...
const evidencePriority = (evidence: Evidence): number => {
  const categoryWeight =
    evidence.category === "confirmation"
      ? 4
...
export const dedupeSameFamily = (evidence: Evidence[]): Evidence[] => {
...
const familyDominantDirection = (
  ...
  evidence
    ...
    .reduce(
      (sum, item) => sum + item.freshnessWeight * item.strength * item.confidence,
      0
    );
...
export const computeResonance = (
  ...
  const primaries = evidence.filter(
    (item) => item.isPrimary && item.freshnessState !== "stale" && item.category !== "risk"
  );
  ...
  const alignedBullishFamilies = [...grouped.values()].filter(
    (items) => familyDominantDirection(items, "bullish") > familyDominantDirection(items, "bearish")
  ).length;

  const alignedBearishFamilies = [...grouped.values()].filter(
    (items) => familyDominantDirection(items, "bearish") > familyDominantDirection(items, "bullish")
  ).length;

  const dominantThemeEntries = primaries
    .filter((item) => item.crossFamilyThemeKey)
    .map((item) => item.crossFamilyThemeKey as string);
  const dominantThemeKey =
    dominantThemeEntries.length === 0
      ? undefined
      : dominantThemeEntries
          .sort(
            (left, right) =>
              dominantThemeEntries.filter((item) => item === right).length -
              dominantThemeEntries.filter((item) => item === left).length
          )[0];
...
export const corroborationBonusForDirection = (
  evidence: Evidence[],
  direction: Exclude<EvidenceDirection, "neutral" | "risk">,
